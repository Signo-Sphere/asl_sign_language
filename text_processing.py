...
# 初始化 T5 模型（用于语法纠错）
t5_tokenizer = AutoTokenizer.from_pretrained("Unbabel/gec-t5_small")
t5_model = AutoModelForSeq2SeqLM.from_pretrained("Unbabel/gec-t5_small")
...
def initialize_marian_model():
    ...
    try:
        ...
        marian_model = MarianMTModel.from_pretrained(model_name, from_tf=True)
...
def clean_translation(text):
    text = re.sub(r'NAME OF TRANSLATORS.*', '', text, flags=re.IGNORECASE)
...
def process_text_with_t5(text):
    inputs = t5_tokenizer.encode("gec: " + text, return_tensors="pt", max_length=512, truncation=True)
...
def process_text_in_sequence(text, use_chatgpt=False):
    ...
    # 使用 T5 模型进行语法纠错
    text = safe_process(process_text_with_t5, text)
    print(f'After T5 Grammar Correction: {text}')

    # 使用 Marian 或 ChatGPT 进行翻译
    if use_chatgpt:
...
# 设置 OpenAI API 密钥
openai.api_key = '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'

# 测试文本处理与翻译
sample_text = "Hello, how are you doing today?"
use_chatgpt = True  # 设置为 False 使用 Marian 模型，True 使用 ChatGPT
